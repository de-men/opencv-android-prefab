 ...
 *
 * Currently a single GStreamingCompiled can process only one opencv_video
 * streat at time. Produce multiple GStreamingCompiled objects to run the
 * same graph on multiple opencv_video streams.
 *
 * @sa GCompiled
 */
...
public:
     ...
     * setSource() to run the graph on a new opencv_video stream.
     *
    ...
    /**
     * @brief Specify an input opencv_video stream for a single-input
     ...
     * input opencv_video stream.
     */
    void setSource(const gapi::wip::IStreamSource::Ptr& s);

    /**
     * @brief Constructs and specifies an input opencv_video stream for a
     ...
     * a opencv_video source was not specified.
     *
